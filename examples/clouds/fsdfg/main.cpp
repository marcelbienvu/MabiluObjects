...
AudioConnection con10(seq, 2, processor, 2);
...
//  processor.dry_wet(ui.get(1));
  ...
  processor.pitch(ui.get(10) * 96.0f - 48.0f);
  processor.stereo_spread(ui.get(3));
  AudioInterrupts();

  if(button.fell()) processor.trigger(1);
  if(button.rose()) processor.trigger(0);
...
void jsonConfig() {
  ...
  doc["clouds"]["pitch"] = ui.get(10) * 96.0f - 48.0f;
