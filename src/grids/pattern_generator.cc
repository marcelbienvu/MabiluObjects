...
uint8_t U8Mix(uint8_t a, uint8_t b, uint8_t x) {
  uint8_t result = (x == 255) ? b : x * b + (255 - x) * a >> 8; 
  return result;
}

uint8_t U8U8MulShift8(uint8_t a, uint8_t b) {
  uint8_t result = a * b >> 8;
  return result;
} 

uint16_t U8U8Mul(uint8_t a, uint8_t b) {
  uint16_t result = a * b; 
  return result;
}
...
void PatternGenerator::LoadSettings() {
  ...
  options_.tap_tempo = 0;
  options_.output_clock = 0;
  options_.output_mode = OUTPUT_MODE_DRUMS;
  options_.gate_mode = 1;
  options_.swing = 0;
  settings_.density[0] = 200;
  settings_.density[1] = 127;
  settings_.density[2] = 255;
  settings_.options.drums.randomness = 0;
  settings_.options.drums.x = 127;
  settings_.options.drums.y = 127;
